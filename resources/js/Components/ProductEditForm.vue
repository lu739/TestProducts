...
function applyInitialProduct(p) {
    form.name = typeof p.name === 'string' ? p.name : String(p.name ?? '');
    form.description =
    ...
    if (rawPrice == null || rawPrice === '') {
        ...
        form.price = Number.isFinite(n) ? n : null;
    }
    const cid = p.category_id ?? p.category?.id;
    form.category_id = typeof cid === 'number' ? cid : cid != null ? Number(cid) : null;
}
...
onMounted(async () => {
    ...
    applyInitialProduct(props.initialProduct);
});

watch(
    () => props.initialProduct?.id,
    (id) => {
        if (id != null && props.initialProduct) {
            applyInitialProduct(props.initialProduct);
        }
    },
);
...
async function submit() {
    ...
    const id = props.initialProduct?.id;
    if (id == null) {
    ...
    try {
        ...
        if (e?.response?.status === 422) {
            ...
            message.value =
                e?.response?.data?.message ?? 'Не удалось сохранить товар';
        }
    } finally {
        processing.value = false;
    }
...
<template>
    <section class="create-form">
        <div class="create-form__head">
            <h1 class="create-form__title">Редактировать товар</h1>
        </div>
